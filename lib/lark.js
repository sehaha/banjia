...
export function buildBriefCard(s, appUrl, room) {
  ...
  const urlBtn = { tag: 'button', text: { tag: 'plain_text', content: '🔗 打开搬家清单' }, type: 'default', behaviors: [{ type: 'open_url', default_url: link }] };
  const elements = [
...
export function buildDoneCard(title, byName, appUrl) {
  const link = appUrl || 'https://banjia-two.vercel.app/';
  return {
    schema: '2.0',
    config: { wide_screen_mode: true },
    ...
    body: {
      elements: [
        ...
        { tag: 'button', text: { tag: 'plain_text', content: '🔗 打开搬家清单' }, type: 'default', behaviors: [{ type: 'open_url', default_url: link }] },
      ],
    },
  };
}

export function buildAlertCard(title, content, appUrl) {
  const link = appUrl || 'https://banjia-two.vercel.app/';
  return {
    schema: '2.0',
    config: { wide_screen_mode: true },
