...
export function buildSystem(ctx) {
  ...
  const langLine = ctx.lang === 'en'
    ? 'Reply in natural, concise English (the user has switched the app to English); lead with the conclusion. Task titles and notes may be in Chinese — you may quote them as-is.'
    : '用简体中文、口语化、简洁地回答，先给结论。';

  return `你是「Move Guide」家庭搬家执行网页里的智能小助手，帮一家人协作完成从 17 Dava 搬到 25 New Dawn 的搬家（目标搬家日 7/9）。${langLine}${ctx.channel === 'lark' ? '你现在在家庭飞书群里被 @ 到，回复要简短、群聊风格。' : ''}
